"""Risk-reward ratio based Take Profit Determiner module."""

from dataclasses import dataclass
from pydantic import BaseModel, Field
from src.strategies.modules.base.pricing import TakeProfitDeterminer, PriceContext
from src.strategies.decorators import register_module


@register_module(
    'take_profit', 'fixed_rr_tp',
    description='리스크/리워드 비율 기반 익절가 결정자',
)
@dataclass(frozen=True)
class RiskRewardTakeProfitDeterminer(TakeProfitDeterminer):
    """Take profit based on risk-reward ratio from stop loss distance."""

    class ParamSchema(BaseModel):
        """Pydantic schema for RR take profit parameters."""
        take_profit_ratio: float = Field(2.0, ge=1.0, le=10.0, description="리스크/리워드 비율")

    @classmethod
    def from_validated_params(cls, params: "RiskRewardTakeProfitDeterminer.ParamSchema") -> "RiskRewardTakeProfitDeterminer":
    ...
    def calculate_take_profit(self, context: PriceContext, stop_loss: float) -> float:
        ...
        tp_distance = sl_distance * self.take_profit_ratio
