...
Tests real WebSocket connection to Binance Testnet and verifies:
...
Requirements:
    - configs/api_keys.ini with valid Testnet API keys
    - use_testnet = true in api_keys.ini
"""
...
async def run_integration_test(duration_seconds: int = 30):
    """
    Run integration test with real Binance Testnet connection

    Args:
        duration_seconds: How long to collect data (default: 30 seconds)
    """
    logger.info("Starting Binance Testnet Integration Test")
    logger.info("=" * 80)
    ...
    try:
        ...
        if not api_config.is_testnet:
            logger.error("❌ ERROR: is_testnet is False! This test requires Testnet.")
            logger.error("   Please set use_testnet = true in configs/api_keys.ini")
            return False
