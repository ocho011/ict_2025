...
class TestAuditLogger:
    ...
    def test_log_event_basic(self, audit_logger):
        ...
        audit_logger.log_event(
            event_type=AuditEventType.ORDER_PLACED, operation="test_operation", symbol="BTCUSDT"
        )
    ...
    def test_log_order_placed(self, audit_logger):
        ...
        order_data = {"symbol": "ETHUSDT", "side": "BUY", "type": "MARKET", "quantity": 0.1}
        response = {"orderId": 123456, "status": "FILLED", "price": "2000.0"}

        audit_logger.log_order_placed(symbol="ETHUSDT", order_data=order_data, response=response)
    ...
    def test_log_order_rejected(self, audit_logger):
        ...
        audit_logger.log_order_rejected(symbol="BTCUSDT", order_data=order_data, error=error)
    ...
    def test_log_retry_attempt(self, audit_logger):
        ...
        error = {"status_code": 429, "error_code": -1003, "error_message": "Rate limit exceeded"}

        audit_logger.log_retry_attempt(
            operation="execute_signal", attempt=2, max_retries=3, error=error, delay=2.0
        )
    ...
    def test_log_rate_limit(self, audit_logger):
        ...
        error = {"status_code": 429, "error_code": -1003, "error_message": "Too many requests"}
        weight_info = {"current_weight": 2100, "weight_limit": 2400}

        audit_logger.log_rate_limit(operation="get_position", error=error, weight_info=weight_info)
    ...
    def test_multiple_log_entries(self, audit_logger):
        ...
        audit_logger.log_event(
            event_type=AuditEventType.LEVERAGE_SET, operation="set_leverage", symbol="BTCUSDT"
        )

        audit_logger.log_event(
            event_type=AuditEventType.MARGIN_TYPE_SET, operation="set_margin_type", symbol="ETHUSDT"
        )

        audit_logger.log_event(
            event_type=AuditEventType.POSITION_QUERY, operation="get_position", symbol="BTCUSDT"
        )
    ...
    def test_json_lines_format(self, audit_logger):
        ...
        for i in range(5):
            audit_logger.log_event(
                event_type=AuditEventType.ORDER_PLACED, operation=f"order_{i}", symbol="BTCUSDT"
            )
    ...
    def test_timestamp_format(self, audit_logger):
        ...
        audit_logger.log_event(event_type=AuditEventType.API_ERROR, operation="test")
