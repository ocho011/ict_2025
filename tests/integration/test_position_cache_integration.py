...
class TestPositionCacheSingleSourceOfTruth:
    ...
    def mock_order_gateway(self):
        ...
        gw = MagicMock()
        gw.get_position = AsyncMock(return_value=Position(
            symbol="BTCUSDT",
            side="LONG",
            quantity=0.001,
            entry_price=50000.0,
    ...
    @pytest.mark.asyncio
    async def test_get_returns_cached_position(self, position_cache_manager, mock_order_gateway):
        """get() should cache and return position, only calling API once within TTL."""
        pos1 = await position_cache_manager.get("BTCUSDT")
        pos2 = await position_cache_manager.get("BTCUSDT")
    ...
    @pytest.mark.asyncio
    async def test_get_fresh_always_calls_api(self, position_cache_manager, mock_order_gateway):
        """get_fresh() should invalidate cache and always fetch from API."""
        pos1 = await position_cache_manager.get("BTCUSDT")
        pos2 = await position_cache_manager.get_fresh("BTCUSDT")
    ...
    @pytest.mark.asyncio
    async def test_websocket_update_reflected_in_get(self, position_cache_manager, mock_order_gateway, mock_config_manager):
    ...
    @pytest.mark.asyncio
    async def test_invalidate_clears_cache_and_cooldown(self, position_cache_manager):
        """invalidate() should clear both position cache and signal cooldown."""
        # Populate cache
        await position_cache_manager.get("BTCUSDT")
