...
class TestEventBusQueues:
    ...
    async def test_signal_queue_raises_timeout_when_full(self, event_bus_with_queues):
        ...
        for i in range(100):
            ...
            await bus.publish(event, queue_type=QueueType.SIGNAL)

        # Verify queue is full
        stats = bus.get_queue_stats()
        assert stats[QueueType.SIGNAL]["size"] == 100
        ...
        with pytest.raises(asyncio.TimeoutError):
            await bus.publish(overflow_event, queue_type=QueueType.SIGNAL)

        # Verify no drops (signal queue never drops)
        stats_after = bus.get_queue_stats()
        assert stats_after[QueueType.SIGNAL]["drops"] == 0
    ...
    async def test_order_queue_blocks_indefinitely_when_full(self, event_bus_with_queues):
        ...
        for i in range(50):
            ...
            await bus.publish(event, queue_type=QueueType.ORDER)

        # Verify queue is full
        stats = bus.get_queue_stats()
        assert stats[QueueType.ORDER]["size"] == 50
    ...
    async def test_get_queue_stats_reflects_current_state(self, event_bus_with_queues):
        ...
        for i in range(5):
            await bus.publish(
                Event(EventType.SIGNAL_GENERATED, {"id": i}, source="test"), queue_type=QueueType.SIGNAL
            )
        for i in range(2):
            await bus.publish(
                Event(EventType.ORDER_PLACED, {"id": i}, source="test"), queue_type=QueueType.ORDER
            )
...
class TestEventBusLifecycle:
    ...
    async def test_integration_start_publish_stop(self):
        ...
        for i in range(3):
            await bus.publish(
                Event(EventType.SIGNAL_GENERATED, {"signal": i}, source="test"), queue_type=QueueType.SIGNAL
            )
