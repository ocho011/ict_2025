...
class TestTradingBotInitialization:
    ...
    def test_initialize_logs_startup_banner(
        ...
        with patch('src.main.BinanceDataCollector'), \
        ...
        assert any('ICT Trading Bot Starting' in call for call in info_calls), \
            "Startup banner missing"
        assert any('TESTNET' in call for call in info_calls), \
            "Environment info missing"
        assert any('BTCUSDT' in call for call in info_calls), \
            "Symbol info missing"
        assert any('mock_sma' in call for call in info_calls), \
            "Strategy info missing"
        assert any('10x' in call for call in info_calls), \
            "Leverage info missing"
    ...
    def test_initialize_passes_candle_callback(
        ...
        with patch('src.main.OrderExecutionManager') as mock_order, \
             patch('src.main.RiskManager'), \
             patch('src.main.StrategyFactory'), \
             patch('src.main.EventBus'):
    ...
    def test_initialize_handles_leverage_failure_gracefully(
        ...
        assert any('Failed to set leverage' in call for call in warning_calls), \
